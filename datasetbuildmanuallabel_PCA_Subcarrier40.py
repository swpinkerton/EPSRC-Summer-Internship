...
### For 5GNT
g1 = glob.glob(path + "/30 April/Activity/_active/x*.csv")              #In order to locate csv files, whose name may be unknown, the glob module is invoked  and glob method is called then. it returns all the csv files list located within the path
g2 = glob.glob(path + "/30 April/Activity/Enter/x*.csv")
g3 = glob.glob(path + "/30 April/Activity/Inactive Active/x*.csv")
g4 = glob.glob(path + "/30 April/Empty/x*.csv")
...
full.to_csv(index=False,path_or_buf='5GNT_Trial1_NoActivity_Dataset_Subcarrier40.csv',header=False)
print 'done'
